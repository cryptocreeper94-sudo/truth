...
- Demo/simulated state is global per transport and used to outrank live scans on the dashboard (hardcoded "2019 Ford F-150" shown instead of VIN). Rule (Jason's explicit requirement): a live adapter ALWAYS wins over demo state; starting any real connection clears demo state on both transports; the demo label carries a "(Demo)" suffix.
- Jason builds APKs locally with Gradle on his laptop (NOT EAS); only a real-car test with the Flight Recorder log can confirm adapter behavior.
